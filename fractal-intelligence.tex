...
\ewreviseddates{April 7, 2026}{August 25, 2026}
...
\begin{abstract}
\noindent Human decompositions often inherit path-dependent conventions. This paper instead asks what a problem is made of: concepts that may transfer across domains, rather than solution steps. Re-deriving its structure may surface inherited assumptions and expose dimensions, categories, and candidate mechanisms that conventional framing leaves unseen. \emph{Fractal intelligence} names a form of cumulative intelligence in which bounded, concept-defined units recursively decompose and reframe problems, compose child work into higher-level capabilities, and preserve evaluated structures for reuse or revision across problems, domains, and scales.

A root Solver bounds the problem space. A problem ascends through adjacent genus-and-differentia specializations to that root, searching for parents and siblings; it then retraces the route before decomposing the specific capability into four-tested constituents. Specialization locates kinds; composition binds differentiated contributions. The five-surface Solver Contract makes resulting units recursively composable; a marginal-value rule governs depth; content-addressed Pathway Memory supports revision and reuse. Parent synthesis states how child contributions, routing, integration, and evaluation realize one cell-like outward capability. A model, human, program, or subtree may supply any interior, and one Solver may serve several parents without changing its contract. Typed rejection can request re-decomposition; recurrent validated structures may become reasoning highways.

A 100-problem mandatory-abstraction construction and an alpha reference implementation show that the proposed topology can be instantiated and structurally checked, but neither tests solution quality. The decisive tests are whether fresh conceptual carves outperform equally persistent conventional approaches under matched compute and memory, whether independent carves recover recurring invariants or complementary utility beyond arbitrary divergence, and whether reuse improves subsequent problem solving. The paper presents a testable theory and architecture, not outcome evidence.
\end{abstract}
...
The rest of this section proceeds in four steps. \S\ref{sec:why-fractal} defines fractal intelligence as a general concept, then presents one Solver-based architecture for realizing it and specifies how problems route among its Solvers. \S\ref{sec:theses} states the seven theses the paper rests on, each paired with a falsification criterion. \S\ref{sec:adds} specifies the six architectural mechanisms that produce them. \S\ref{sec:contributions} positions the work against its intellectual ancestors and names what this paper adds that existing frameworks do not.
...
Fractal intelligence denotes the broader theoretical category. The Solver architecture developed here gives that pattern a concrete form through a root boundary, typed contracts, acceptance gates, Pathway Memory, and a marginal-value rule. These mechanisms specify this paper's realization without exhausting the category.
...
The foundation of this realization is first-principles thinking. At every node, the decomposition asks the same question: what are the irreducible dimensions of this concept? The term \emph{fractal} is used in an organizational rather than strictly geometric sense: the same relationship recurs at multiple scales. A problem can be decomposed into conceptual subproblems; each subproblem can be decomposed again; and a solved substructure can itself function as a component of a larger one. The five-surface Solver Contract expresses this recurrence uniformly at every level. The resolution increases; the organizational pattern remains.
...
Candidate nodes arise in two ways: as specialization membranes located along the upward abstraction route, or as compositional Solvers generated by the downward four-test search operator developed in \S\ref{sec:algorithm}. Evaluated solution graphs enter the archive; recurrent, validated, or uniquely useful subgraphs may be promoted into the active serving layer, and Pathway Memory (\S\ref{sec:partition}) informs later routing under the marginal-value rule (\S\ref{sec:mvr}). The architecture permits alternative search operators, catalogues, and routing policies. The ten protocols are worked examples whose value does not require convergence on a definitive ontology.
...
\emph{Gates that request conceptual restructuring.} Rejection is a typed diagnostic naming the structural failure and asking the upstream Solver to change its approach rather than merely rephrase it. The retirement-system traces in Section~\ref{sec:synergy} illustrate an analogous redirection path; they do not implement a Solver Contract gate or isolate the controller's causal effect from the companion system's other differences.
...
\centering
\small
\caption{Illustrative role-based task pipeline vs.\ the proposed Solver Tree.}
...
The concept--task distinction therefore concerns the relation asserted by a decomposition, not the abstractness of a label. Task decomposition partitions a selected procedure; conceptual decomposition proposes what the target capability or problem class is made of. A reusable task method remains task-organized if it encodes a route, while an apparent action can become a conceptual subject if the system asks what capability that action realizes and what constitutes it. Selecting the same stored node or content-addressed Solver again is \emph{structural reuse}; claiming that competence transferred requires the stronger evidence that its contract remained valid and its use improved or preserved relevant outcomes in the new context.
...
\end{tikzpicture}}
\caption{Proposed conceptual decomposition of the economy. White nodes are candidate axes; gray nodes are candidate sub-concepts that recurse further.}
...
The tree omits implementation actions such as raising interest rates, adjusting taxes, creating jobs, or building infrastructure. Those tasks emerge inside the conceptual structure: ``raise interest rates'' belongs within Stability $\to$ Feedback Regulation, while ``adjust taxes'' belongs within Distribution $\to$ Fairness $\to$ Distributive Fairness. The actions are economy-specific; the proposed conceptual axes are intended to recur across economic systems.
...
\end{tikzpicture}}
\caption{Proposed conceptual decomposition of education, with candidate reusable sub-concepts at depth two.}
...
\end{tikzpicture}}
\caption{Proposed conceptual decomposition of game design into five dimensions.}
\label{fig:game-tree}
\end{figure}
...
The architecture therefore separates three layers. An \emph{inclusive archive} retains evaluated solution graphs, including failed routes when they provide negative knowledge. A \emph{selective serving layer} promotes recurrent, validated, or niche-leading nodes and subgraphs into active infrastructure; Pathway Memory learns when to route through each. An \emph{experimental frontier} receives a bounded budget for fresh, convention-resistant carves when similarity is low, routing confidence is poor, frame errors recur, stakes are high, or an exploration schedule fires. Cheap carve sketches can be screened before full execution. This is a portfolio in memory: nothing is epistemically discarded, but familiar problems normally exploit one evidence-backed incumbent rather than execute every retained carve.
...
The contract's intellectual ancestry is precise. Hoare~\cite{hoare1969} established verification through preconditions and postconditions. Meyer~\cite{meyer1992dbc} operationalized Design by Contract. Liskov and Wing~\cite{liskov1994} extended behavioral subtyping. Findler and Felleisen~\cite{findler2002} showed that contracts compose for higher-order functions---the recursive pattern Solvers require. The proposed extension here is that the boundary is not merely a validator but can return a typed request for upstream restructuring when the gate rejects. When that request changes the working carve rather than merely triggering another attempt, the boundary participates in ontological accommodation (Section~\ref{sec:jumps}). The five surfaces defined by the Solver Contract are shown in Table~\ref{tab:contract}.
...
\noindent The \texttt{schema} field references a content-addressed Sema specification (\texttt{sema:AcceptSpec\#e4f1}), allowing a receiver to verify that both parties reference the same immutable criteria. The hash establishes identity and integrity, not that the criteria are correct or sufficient. The \texttt{status: partial} option enables the anytime property: a Solver that exhausts its budget returns the best result so far.
...
A Solver may begin as a general model operating within a narrow semantic neighborhood. Its specificity comes from execution harnesses, tools, weight adaptation, and its position in the graph: a \texttt{StabilityRegulationSolver} receives a typed subproblem isolated by upstream decomposition and emits an artifact filtered by an \texttt{AcceptSpec}. Repeated use may then justify progressively stronger specialization under the marginal-value rule. A prompted role can therefore develop into a specialized system when its invocation history supports the investment.

\subsection{Sema: Content-Addressed Concepts}
\label{sec:sema}

Collective thinking requires a shared repertoire of reusable cognitive strategies---patterns that prescribe \emph{how} to think about a class of problems, not merely what a concept denotes. Sema~\cite{westerberg2026sema} provides this by content-addressing pattern definitions: the cryptographic hash of a specification becomes its identifier, so two Solvers referencing the same hash are guaranteed to reference the same immutable specification---the same principle by which Git content-addresses code, applied to cognitive operations rather than source files. The hash guarantees specification-byte identity only; interpretation, execution, and semantic discovery require additional mechanisms. Content identity needs no central registry, while discovering a compatible specification still depends on an unresolved semantic matching procedure; neither the construction artifact nor the reference implementation validates semantic equivalence. \emph{Semantic identity}---whether differently encoded specifications express the same operative invariants---is therefore an unresolved equivalence claim, not a property of the hash. The proposed architecture's operational components are formalized as Sema patterns (Table~\ref{tab:mapping}), making the protocol self-describing.
...
\begin{table}[h]
\centering
\caption{Mapping from architectural components to content-addressed Sema patterns.}
\label{tab:mapping}
\small
\begin{tabular}{@{}llll@{}}
\toprule
...
A \texttt{TextCategorizationSolver} assigns each paragraph a conceptual type---definition, mechanism, example, property, motivation, claim---and flags structural violations such as several consecutive paragraphs performing the same rhetorical function. This operates on the concepts underneath the text rather than on surface wording alone.
...
A simple greeting may resolve in one shallow pass. An ambiguous, high-stakes situation can justify deeper recursion: \texttt{EmotionSolver} spawns sub-solvers to model conflicting emotional signals, \texttt{IntentSolver} distinguishes the stated request from the underlying need, and \texttt{BoundarySolver} tightens its gate. When the words say ``I'm fine'' but the trajectory suggests otherwise, \texttt{EmotionSolver} may spawn competing \texttt{EmotionalHypothesisSolvers} for masked distress, trust-testing, and genuine reassurance. An acceptance gate then composes rather than prematurely collapses those hypotheses. The contrast illustrates the marginal-value rule: routine interactions remain shallow while uncertain, consequential ones receive more analysis.
...
\emph{Open commons.} Every Solver shares its Pathway Memory with every other instance of the same concept. Company~A's financial services insights immediately benefit Company~B's healthcare instance. This maximizes the pool of available signal---the trunk can thicken fastest when everyone contributes---but also expands privacy, capture, negative-transfer, and common-mode risks.

\emph{Federated sharing.} Solvers share structural insights---which sub-concepts work, what depths are productive, which decompositions fail---but not domain-specific data. Company~A learns that \texttt{IncentiveAlignmentSolver} decomposes better into intrinsic, extrinsic, and structural dimensions than into individual and collective. That structural insight transfers without revealing proprietary financial services data. The shared architectural knowledge improves while the competitive advantage from domain-specific application is retained.
...
This paper draws on and departs from six adjacent literatures: conceptual decomposition methods, boundary formalization between agents, cognitive architectures for specialization, generator-verifier pipelines, outcome-grounded learning, and economic governance of compute. The central question for evaluating related work is relational rather than terminological: does a framework partition a chosen procedure into actions, stages, or roles, or propose constitutive capabilities through which several procedures and instances can route (Section~\ref{sec:concept-task})? Persistence and abstract labels alone do not decide the question. Each body of work is evaluated through this lens.
...
A candidate mitigation is a \emph{root-intent skip-connection}: every leaf node receives, in addition to its strict typed Task, a read-only pointer or low-dimensional representation of the original \texttt{RootSolver} Manifest. A soft-prompt or cross-attention mechanism could condition the leaf on this representation without passing the full root prompt through every intermediate Solver. The structural analogue is a residual connection in a deep network: a thin trace of the original intent bypasses intermediate boundaries. Whether such a trace preserves useful global context without reintroducing instruction leakage remains to be tested.
...
The central claim concerns leverage rather than a replacement for stronger models. As the scientific method let discoveries accumulate and correct one another, \emph{fractal intelligence} denotes the broader possibility that machine reasoning might do the same with solution structures; the Solver architecture develops one possible realization. Whether such a network distributes cognitive power or concentrates it depends on how, and by whom, its goals, gates, routing, and memory are governed. The open question is whether cumulative, recursively restructuring intelligence can operate globally and build an internet of reasoning from structures that survive use.
